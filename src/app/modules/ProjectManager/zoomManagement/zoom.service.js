...
export const ZoomService = {
    ...
    // 🔹 1. Generate Zoom Authorization URL
    generateZoomAuthUrl: async (userId) => {
        try {
            const { ZOOM_CLIENT_ID, ZOOM_REDIRECT_URI } = envVars;
            ...
            const url = `https://zoom.us/oauth/authorize?response_type=code&client_id=${ZOOM_CLIENT_ID}&redirect_uri=${encodeURIComponent(ZOOM_REDIRECT_URI)}&state=${userId}&scope=recording:read user:read`;
    ...
    handleZoomCallback: async (code, userId) => {
        try {
            const { ZOOM_CLIENT_ID, ZOOM_CLIENT_SECRET, ZOOM_REDIRECT_URI } = envVars;

            if (!code || !userId) {
                throw new Error("Missing authorization code or userId.");
            }
            ...
            const savedToken = await prisma.zoomToken.upsert({
                where: { connectedUserId: userId },
                update: {
            ...
            throw new Error("Failed to handle Zoom OAuth callback.");
        }
    },

    // 🔹 3. Refresh Access Token
    refreshZoomAccessToken: async (userId) => {
        try {
            const { ZOOM_CLIENT_ID, ZOOM_CLIENT_SECRET } = envVars;

            const tokenData = await prisma.zoomToken.findUnique({
                where: { connectedUserId: userId },
            });
    ...
    // 🔹 4. Get Valid Access Token (Auto refresh)
    getValidZoomAccessToken: async (userId) => {
        try {
            const tokenData = await prisma.zoomToken.findUnique({
                where: { connectedUserId: userId },
            });
            ...
            const isExpired = new Date() >= new Date(tokenData.tokenExpiry);

            if (isExpired) {
                return await ZoomService.refreshZoomAccessToken(userId);
            }

            return tokenData.accessToken;

        } catch (error) {
            console.error("Error getting valid Zoom token:", error.message);
            throw error;
        }
    },
};